...
def affine_backward(dout, cache):
    ...
    # print(x.shape)
    # print(dout.shape)
    # print('w.backward.shape', w.shape)
...
def batchnorm_backward(dout, cache):
    ...
    mu = x - mean             
    sample_std_inv = 1.0 / np.sqrt(var + eps)    
    dvar = -0.5 * np.sum(dout_media * mu, axis=0, keepdims=True) * sample_std_inv**3
    
    # print(np.mean(mu, axis=0, keepdims=True))
    dmean = -1.0 * np.sum(dout_media * sample_std_inv, axis=0, keepdims=True) #- 2.0 * var * np.mean(mu, axis=0, keepdims=True)
    dx1 = dout_media * sample_std_inv
    dx2 = 2.0/N * dvar * mu
    dx = dx1 + dx2 + 1.0/N * dmean
    
    dgamma = np.sum(dout * out_media, axis=0)
    #print('dgamma in layers:', dout.shape)
...
def batchnorm_backward_alt(dout, cache):
    ...
    N = x.shape[0]
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(out_media * dout, axis=0)
    dout_media = dout * gamma
     
    dvar = -0.5 * np.sum(dout_media * (x - mean), axis=0, keepdims=True) * ( 1.0 / np.sqrt(var + eps))**3
    dmean = -1.0 * np.sum(dout_media * ( 1.0 / np.sqrt(var + eps)), axis=0, keepdims=True) - 2.0 * var * np.mean((x - mean), axis=0, keepdims=True)
    dx = 1/np.sqrt(var + eps)*dout_media + dvar*2.0/N*(x-mean) + 1.0/N*dmean
...
def dropout_backward(dout, cache):
    """
    Perform the backward pass for (inverted) dropout.e
    Inputs:
    - dout: Upstream derivatives, of any shape
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    OUT_H = int(1 + (H + 2 * pad - HH) / S)
    OUT_W = int(1 + (W + 2 * pad - WW) / S)
        ...
        for j in range (F):
            for m in range(OUT_H):
                for n in range(OUT_W):
                    out[i,j,m,n] = np.sum(x_pad[i, :, m * S:m * S + HH, n * S:n * S + WW] * w[j, :]) + b[j]
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(N):
        for j in range (C):
            for m in range(OUT_H):
                for n in range(OUT_W):
                    window = x[i, j, m*S:HH+m*S, n*S:WW+n*S] 
                    out[i, j, m, n] = np.max(window)
