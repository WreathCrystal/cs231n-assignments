...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # normalization layer. You should pass self.bn_params[0] to the forward passs
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            config['fc' + str(L)] = cache
            
            if not L == self.num_layers:
                if self.normalization == 'batchnorm':
                    N, D = out.shape
                    bn_param.update({'running_mean':np.zeros(D, dtype=out.dtype)})
                    bn_param.update({'running_var':np.zeros(D, dtype=out.dtype)})
                    gamma = self.params['gamma' + str(L)]
                    beta = self.params['beta' + str(L)]                    
                    out, cache = batchnorm_forward(out, gamma, beta, bn_param)
                    config['bn'+str(L)] = cache

                out, cache = relu_forward(out)
                config['relu' + str(L)] = cache
                
                if self.use_dropout:                    
                    out, cache = dropout_forward(out, self.dropout_param)                    
                    config['dropout'+ str(L)] = cache
        ...
        for i in range(self.num_layers, 0, -1):                   
            if not i == self.num_layers:
                """    """ 
                if self.use_dropout:
                    cache = config['dropout' + str(i)]
                    dout = dropout_backward(dout, cache)
               
                
                cache = config['relu' + str(i)]
                dout = relu_backward(dout, cache) 
                
                if self.normalization == 'batchnorm':
                    cache = config['bn' + str(i)]                    
                    dout, dgamma, dbeta = batchnorm_backward(dout, cache)   
                    grads['gamma' + str(i)] = dgamma
                    grads['beta' + str(i)] = dbeta
            
            cache = config['fc' + str(i)]
            dout , dw, db = affine_backward(dout, cache)
            dw +=  2*0.5*self.reg * self.params['W' + str(i)]
